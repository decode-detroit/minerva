...
use super::{ReadEvents, WriteEvent, ItemId, COMM_ERROR, READ_ERROR};
...
impl ComedyComm {
    ...
    fn escape(message: Vec<u8>) -> Vec<u8> {
        ...
        for character in message {
            ...
            if character == FIELD_SEPARATOR || character == COMMAND_SEPARATOR || character == ESCAPE_CHARACTER || character == NULL_CHARACTER {
    ...
    fn write_event_now(&mut self, id: ItemId, data1: u32, data2: u32) -> Result<(), ()> {
        ...
        let mut tmp = Vec::new();
        if let Err(_) = tmp.write_u32::<LittleEndian>(id.id()) { return Err(()); }
            
        // Escape the new argument and then add it
        if let Err(_) = bytes.write(ComedyComm::escape(tmp).as_slice()) { return Err(()); }
        
        // Add the separator and convert each argument to a character vector
        bytes.push(FIELD_SEPARATOR);
        let mut tmp = Vec::new();
        if let Err(_) = tmp.write_u32::<LittleEndian>(data1) { return Err(()); }
        ...
        bytes.push(FIELD_SEPARATOR);
        let mut tmp = Vec::new();
        if let Err(_) = tmp.write_u32::<LittleEndian>(data2) { return Err(()); }
        ...
        if let Err(_) = self.port.write(bytes.as_slice()) { return Err(()); }
        ...
        if self.outgoing.len() > 0 {
            ...
            match self.last_ack {
                ...
                None => {
                    ...
                    self.write_event_now(id.clone(), data1.clone(), data2.clone()).unwrap_or(());
                },
