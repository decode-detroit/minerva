...
impl Mercury {
    ...
    ///
    pub fn new(path: &PathBuf, baud: usize, use_checksum: bool, allowed_events: Option<FnvHashSet<ItemId>>, polling_rate: u64) -> Result<Self> {
...
impl EventConnection for Mercury {
    ...
    fn write_event(&mut self, id: ItemId, data1: u32, data2: u32) -> Result<()> {
        ...
        if let Some(ref mut port) = self.port {
            ...
            if Mercury::write_event_now(port, self.use_checksum, id, data1, data2).is_err() {
                is_unavailable = true;
            }
        ...
        if is_unavailable {
            if let Some(bad_port) = self.port.take() {
                ...
                // Return the error
                return Err(anyhow!("Lost connection to Mercury port."));
            }
        }
