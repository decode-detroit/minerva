...
pub use self::config::{StatusDescription, FullStatus};
...
use self::item::{ItemPair, ItemId, ItemDescription};
use self::event::{EventDetail, EventDelay, NewScene, ModifyStatus, TriggerEvents, SaveData, GroupedEvent, EventUpdate, UpcomingEvent};
use self::queue::{Queue, ComingEvent};
...
impl EventHandler {
    ...
    ///
    pub fn new(config_path: PathBuf, general_update: GeneralUpdate, log_failure: bool) -> Option<EventHandler> {
        ...
        let backup = match BackupHandler::new(general_update.clone(), config.identifier(), config.server_location()) {
            Some(b) => b,
            None => return None,
        };
    ...
    pub fn upcoming_events(&self) -> Option<Vec<UpcomingEvent>> {
        ...
        if let Some(mut events) = self.queue.list_events() {
            ...
            for event in events.drain(..) {
                ...
                coming_events.push(UpcomingEvent {start_time: event.start_time,
                                                  delay: event.delay,
    ...
    pub fn get_state_description(&self, status_id: &ItemId) -> Option<ItemPair> {
        ...
        if let Some(state) = self.get_state(status_id) {
            return Some(ItemPair::from_item(state.clone(), self.get_description(&state)));
        }
    ...
    pub fn modify_status(&mut self, status_id: &ItemId, new_state: &ItemId) {
        ...
        if self.config.modify_status(status_id, new_state).is_ok() {
            ...
            self.queue.add_event(EventDelay::new(None, new_state.clone()));
        }
    }
    ...
    pub fn adjust_event(&self, event_id: ItemId, start_time: Instant, new_delay: Duration) {
        ...
        self.queue.adjust_event(ComingEvent {event_id, start_time, delay: new_delay });
    }
    ...
    fn unpack_event(&mut self, event_detail: EventDetail) {
        ...
        match event_detail {
            ...
            GroupedEvent { status_id, event_map } => {
...
mod tests {
    ...
    fn handle_events() {
        ...
        let mut event_handler = EventHandler::new(PathBuf::from("examples/testing_config.mnv"), tx, unused).unwrap();
        
        // Select the correct scene
        event_handler.config.choose_scene(ItemId::new(100).unwrap());
        
        // Define the first two event delays
        let event1_delay = EventDelay::new(Some(Duration::from_millis(100)), ItemId::new(1).unwrap());
        let event2_delay = EventDelay::new(Some(Duration::from_millis(200)), ItemId::new(2).unwrap());
        
        // Trigger the first two events
        event_handler.queue.add_events(vec![event1_delay, event2_delay]);
        
        // Create the test vector
        let test = vec!(EventUpdate::Update("Got Data: 1".to_string()),
    ...
    fn link_events() {
        ...
        use std::time::Duration;
        use self::event::EventDelay;
        use self::item::{DisplayWith, Display, Hidden};
        ...
        // Define the first two event delays
        let event5_delay = EventDelay::new(Some(Duration::from_millis(200)), ItemId::new(5).unwrap());
        let event8_delay = EventDelay::new(Some(Duration::from_millis(500)), ItemId::new(8).unwrap());
        
        // Trigger the first and last event
        event_handler.queue.add_events(vec![event5_delay, event8_delay]);

        // Create the test vector
        let test = vec!(EventUpdate::Broadcast(ItemPair::new(5, "Load Immediate Events (6 & 7)", Display).unwrap()),
                        EventUpdate::Broadcast(ItemPair::new(15, "Load Delayed Events (5)", Hidden).unwrap()),
                        EventUpdate::Current(ItemPair::new(6, "Load Events Or Save Data (Grouped Event)", DisplayWith(ItemId::new(10).unwrap())).unwrap()),
                        EventUpdate::Update("Got Data: 7".to_string()),
                        EventUpdate::Current(ItemPair::new(7, "Save Data 7", Display).unwrap()),
                        EventUpdate::Broadcast(ItemPair::new(5, "Load Immediate Events (6 & 7)", Display).unwrap()),
                        EventUpdate::Broadcast(ItemPair::new(15, "Load Delayed Events (5)", Hidden).unwrap()),
                        EventUpdate::Current(ItemPair::new(6, "Load Events Or Save Data (Grouped Event)", DisplayWith(ItemId::new(10).unwrap())).unwrap()),
                        EventUpdate::Update("Got Data: 7".to_string()),
                        EventUpdate::Current(ItemPair::new(7, "Save Data 7", Display).unwrap()),
                        EventUpdate::Status(ItemPair::new(10, "Test Event Group - Loop Or Save", Hidden).unwrap(), ItemDescription::new("Currently Saving Data", Display)),
