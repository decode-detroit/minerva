...
use super::super::item::{ItemId, ItemDescription, ItemPair};
...
impl StatusHandler {
    ...
    pub fn new(update_line: GeneralUpdate, status_map: StatusMap) -> StatusHandler {
        ...
        StatusHandler { status_map, update_line }
    }
    ...
    where F: FnMut(&ItemId) -> ItemDescription {
        ...
        for status_id in id_vec {
            ...
            let status_description = match self.status_map.get(&status_id) {
                ...
                Some(detail) => {
                    ...
                    let current_pair = ItemPair::from_item(detail.current(), get_description(&detail.current()));
                    
                    // Rapackage the allowed states
                    let mut allowed_pairs = Vec::new();
                    for state in detail.allowed() {
                        allowed_pairs.push(ItemPair::from_item(state.clone(), get_description(&state)));
                    }
                    
                    // Return the new status description
                    StatusDescription { current: current_pair, allowed: allowed_pairs }
                },
...
use self::StatusDetail::{MultiState, CountedState};
...
impl StatusDetail {
    ...
    pub fn allowed(&self) -> Vec<ItemId> {
        match self {
            ...
            &CountedState { ref trigger, ref anti_trigger, ref reset, .. } => {
    ...
    pub fn update(&mut self, new_state: ItemId) -> bool {
        match self {
            ...
            // The multistate variant
            &mut MultiState { ref mut current, ref allowed, .. } => {
            ...
            // The countedstate variant
            &mut CountedState { ref mut current, ref mut count, ref reset, ref default_count, ref trigger, ref anti_trigger, .. } => {
