...
use super::system_interface::{SystemSend, SystemUpdate, InterfaceUpdate, UpdateConfig, UpdateWindow, UpdateStatus, UpdateNotifications, UpdateQueue, Notify, Description, DetailToModify, EditMode, DebugMode};
...
impl UserInterface {
    ...
    /// program.
    ///
    pub fn new(application: &gtk::Application, window: &gtk::ApplicationWindow, system_send: SystemSend, interface_send: mpsc::Sender<InterfaceUpdate>, interface_receive: mpsc::Receiver<InterfaceUpdate>) -> UserInterface {
        
        // Create a new interface abstraction and add the top element to the window
        let edit_mode = Rc::new(RefCell::new(false));
        let interface_abstraction = InterfaceAbstraction::new(&system_send, window, edit_mode.clone());
        ...
        let user_interface = UserInterface { interface_abstraction, edit_mode, system_send, interface_send, window: window.clone() };
    ...
    pub fn check_updates(&self, interface_update: &mpsc::Receiver<InterfaceUpdate>) {
        ...
        loop {
            ...
            match update {
                ...
                UpdateConfig { scenes, full_status } => { 
                ...
                UpdateWindow { current_scene, window } => interface.update_window(current_scene, window, &self.system_send),
                
                // Update the state of a particular status
                UpdateStatus { status_id, new_state } => interface.update_state(status_id, new_state),
                
                // Update the notifications in the notification window
                UpdateNotifications { notifications } => interface.update_notifications(notifications),
                ...
                DetailToModify { event_id, event_detail } => interface.launch_edit_event(&self.system_send, Some(event_id), Some(event_detail)),
