...
use self::special_windows::{EditDialog, StatusDialog, JumpDialog, TriggerDialog, InfoDialog, EditEventDialog};
use super::utils::clean_text;
use super::super::system_interface::{SystemSend, EventWindow, ItemPair, UpcomingEvent, Notification, FullStatus, EventDetail};
...
impl InterfaceAbstraction {
    ...
    /// clarity).
    ///
    pub fn new(system_send: &SystemSend, window: &gtk::ApplicationWindow, edit_mode: Rc<RefCell<bool>>) -> InterfaceAbstraction {
    ...
    ///
    pub fn update_window(&mut self, current_scene: ItemPair, window: EventWindow, system_send: &SystemSend) {
        self.events.update_window(current_scene, window, system_send);
    }
    ...
    pub fn notify(&mut self, new_text: &str) {
        ...
        let message = format!("\t\t{} — {}", timestr, clean_text(&new_text, NOTIFY_LIMIT, false, false, false));
    ...
    pub fn update_state(&mut self, status_id: ItemPair, new_state: ItemPair) {
        ...
        self.events.update_state(status_id.clone(), new_state.clone());
    ...
    ///
    pub fn launch_edit_event(&self, system_send: &SystemSend, event_id: Option<ItemPair>, event_detail: Option<EventDetail>) {
        self.edit_event_dialog.launch(system_send, event_id, event_detail);
    }
}
