...
use super::super::super::system_interface::{SystemSend, EventChange, ItemPair, DisplayControl, DisplayWith, DisplayDebug, UpcomingEvent};
...
impl TimelineAbstraction {
    ...
    pub fn new(system_send: &SystemSend, window: &gtk::ApplicationWindow) -> TimelineAbstraction {
        ...
        let timeline_info = Rc::new(RefCell::new(
                            TimelineInfo { start: Instant::now(),
        ...
        let adjustment = TimelineAdjustment { timeline_events: timeline_events.clone(), system_send: system_send.clone() };
    ...
    pub fn update_events(&mut self, mut events: Vec<UpcomingEvent>) {
        ...
        for event in events.iter() {
            ...
            if event.start_time < timeline_info.start {
                timeline_info.duration = timeline_info.duration + (timeline_info.start - event.start_time); // modify the old duration to match the current start time
        ...
        for event in events.drain(..) {
            ...
            if let Some(existing_event) = timeline_events.get_mut(&TimelineEvent::new_unique_id(&event.event, &event.start_time)) {
    ...
    ///
    fn draw_timeline(timeline_events: Rc<RefCell<FnvHashMap<String, TimelineEvent>>>, timeline_info: Rc<RefCell<TimelineInfo>>, area: &gtk::DrawingArea, cr: &cairo::Context) -> Inhibit {
        ...
        cr.set_font_matrix(cairo::Matrix{ xx: (FONT_SIZE / width), yy: (FONT_SIZE / height), xy: 0.0, yx: 0.0, x0: 0.0, y0: 0.0 }); 
        ...
        for (_, event) in events.iter_mut() {
            ...
            if TimelineAbstraction::draw_event(cr, timeline_info.clone(), event, width, &event_locations) {
            ...
            // Otherwise, draw a placeholder
                ...
                for spot in placeholders.iter() {
                    ...
                    if (location > (*spot - label_width) && location < *spot) || (location_width > (*spot - label_width) && location_width < *spot) {
        ...
        if !info.is_stale {
            ...
            cr.show_text(format!(" {:02}:{:02}:{:02}", time.tm_hour, time.tm_min, time.tm_sec).as_str());
        }
    ...
    ///
    fn draw_event(cr: &cairo::Context, timeline_info: Rc<RefCell<TimelineInfo>>, event: &mut TimelineEvent, width: f64, event_locations: &Vec<f64>) -> bool {
        ...
        if text_visible {
            ...
            // Write the event text and the remaining time
            cr.move_to(location - (LABEL_ADJUSTMENT / width), 0.3);
            let text = clean_text(&event.event.description(), TIMELINE_LIMIT, false, false, false);
